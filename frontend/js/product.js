...
async function loadProductDetails() {
    try {
        ...
        document.title = `${product.name} — Sri Maruthi textiles`;
        ...
        if (mediaItems[0].type === 'video') {
            ...
            firstMediaHtml = `<img src="${mediaItems[0].url}" class="main-image-preview" id="main-image-preview" alt="${product.name}" onerror="this.onerror=null; this.src='${fallbackImage}'">`;
        }
            ...
            mediaItems.forEach((item, idx) => {
                ...
                galleryHtml += `
                    <div class="thumbnail-item ${idx === 0 ? 'active' : ''} ${isVideo ? 'video-thumbnail' : ''}" data-index="${idx}">
                        <img src="${thumbSrc}" alt="${product.name} Thumbnail" loading="lazy" onerror="this.onerror=null; this.src='${fallbackImage}'">
                        ${isVideo ? `
